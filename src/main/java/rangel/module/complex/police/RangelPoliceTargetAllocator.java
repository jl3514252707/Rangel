...
public class RangelPoliceTargetAllocator extends PoliceTargetAllocator {
  ...
  public RangelPoliceTargetAllocator(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData) {
  ...
  public PoliceTargetAllocator resume(PrecomputeData precomputeData) {
    ...
    for (EntityID id : this.worldInfo
        .getEntityIDsOfType(StandardEntityURN.POLICE_FORCE)) {
  ...
  private PoliceForceInfo update(PoliceForceInfo info,
      MessagePoliceForce message) {
  ...
  private List<StandardEntity>
      getActionAgents(Map<EntityID, PoliceForceInfo> infoMap) {
    ...
    for (StandardEntity entity : this.worldInfo
      ...
      if (info != null && info.canNewAction
          && ((PoliceForce) entity).isPositionDefined()) {
        result.add(entity);
      }
    }
    return result;
  ...
  private Map<EntityID, EntityID>
      convert(Map<EntityID, PoliceForceInfo> infoMap) {
